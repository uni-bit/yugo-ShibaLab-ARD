...
`docs/` フォルダにコードリーディング用の詳細ガイドを用意しています。

| ガイド | 内容 |
|--------|------|
| [**全体構成ガイド**](docs/code_reading_guide.md) | フォルダ構成マップ・全機能一覧・おすすめ読み順 |
| [**スマホジャイロセンサー入力**](docs/feature_gyro_sensor.md) | UDP/OSC 受信・座標変換・キャリブレーションの実装詳細 |
| [**二画面 Off-Axis 投影**](docs/feature_dual_screen_projection.md) | 非軸投影行列の数学・マルチディスプレイ構成 |
| [**スポットライト照射判定**](docs/feature_spotlight_sensor.md) | 角度/距離ベースの照射量計算ロジック |
| [**ステージ切り替え制御**](docs/feature_stage_sequence.md) | 複数ステージの排他管理・フェード遷移 |
| [**ステージ音声制御**](docs/feature_stage_audio.md) | BGM 自動切り替え・イベント連動 SE |
| [**スポットライト連動シェーダー**](docs/feature_spotlight_shader.md) | ライトで照らした部分だけ表示するシェーダー |

> 各機能ガイドには **「自分のプロジェクトで実装するには」** セクションがあり、他プロジェクトへの転用手順も記載しています。
